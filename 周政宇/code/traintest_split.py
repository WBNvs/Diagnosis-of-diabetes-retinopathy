...
split_idx = int(img_paths.shape[0] * 0.8)
...
for i in range(train_paths.size):
	train_paths_[i] = train_paths[i][8:] #patches/
	train_paths_[i] = "labels/" + train_paths_[i]
	print (train_paths_[i])

print ("split")

for i in range(test_paths.size):
	test_paths_[i] = test_paths[i][8:]
	test_paths_[i] = "labels/" + test_paths_[i]
	print (test_paths_[i])
